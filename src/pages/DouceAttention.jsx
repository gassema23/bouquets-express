...
function DouceAttention() {
  return (
    <>
      <PageBanner
        pageTitle={"Douce attention"}
        breadcrumb={["Services", "Douce attention"]}
        banner={banner}
      />
      <main className="md:container md:mx-auto px-4 md:px-0 my-10">
        ...
        <section className="my-28 ">
          ...
          <div className="px-4 md:px-0 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 grid gap-5 mt-12">
            {Products.filter((product) => product.category == "Douce attention")
              .slice(0, 4)
              .map((product) => (
